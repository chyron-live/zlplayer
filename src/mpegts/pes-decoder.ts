...
export default class PESDecoder {
  ...
  public add(packet: Uint8Array): Uint8Array[] | null {
    ...
    if (payload_unit_start_indicator(packet)) {
      ...
      const length: number = this.chunks?.expect() ?? ((PES_packet_length(packet,offset) === 0 ? 0 : PES_packet_length(packet, offset) + 6));
      ...
      this.chunks.push(packet.subarray(offset, Math.min(offset + rest, PACKET_LENGTH)));
      ...
      this.chunks.push(packet.subarray(begin));
